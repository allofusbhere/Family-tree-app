...
(function(){
  ...
  const historyStack = [];
  let currentId = null;
  let mode = 'anchor'; // 'anchor' | 'grid'
  let gridType = null; // 'siblings' | 'children' | 'parents'
  ...
  // --------- Gesture handling (pointer with touch/mouse fallback) ---------
  ...
  function onEnd(x,y){
    ...
    if (ax > ay){
      ...
      if (dy < 0){
        // parents grid (both centered if both exist)
