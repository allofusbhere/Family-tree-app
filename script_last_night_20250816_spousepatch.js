...
// --- Extended spouse parser ---
function resolveSpouseTarget(currentId){
  const p = getIdParts(currentId);
  // If this is already an extended spouse reference: jump to partnerHint
  if(p.isSpouse && p.partnerHint){
    return p.partnerHint;
  }
  // If anchor is base person: try to load extended spouse alias
  if(!p.isSpouse){
    return p.baseId + '.1';
  }
  return p.baseId;
...
async function handleSpouseSwipe(){
  const target = resolveSpouseTarget(state.anchorId);
